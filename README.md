...
- 🧠 **AI 模糊启发式搜索** - 即使跨盘符漂移，配置向导也能依靠特征打分和深度权重精准锁定 `main.py` 等实际业务程序
- 🌳 **原生深度进程追踪** - 解决 VBS/Cmd 封装导致的孤儿进程失控，真正管理深层生命周期树
- ⚡ **毫秒级 GUI 数据同步** - 可视化界面支持打字实时反馈与快速数据入集，配置更流畅
...
配置向导内嵌先进的 `findBestCandidate` 模糊打分算法，会自动搜索：
- **Python 应用**: 虚拟环境、系统 Python，并依靠上下文奖励机制自动排除 `site-packages` 等海量同名干扰脚本
- **Node.js 应用**: 项目中的 `.js` 关联业务执行树
- **无视盘符迁移**: 即使整个绿色部署包移动覆盖到全新的操作系统盘符，进程管家仍能像人眼一样准度秒出各服务进程链路。
