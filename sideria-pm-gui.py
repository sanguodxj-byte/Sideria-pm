...
class SideriaPMGUI:
    ...
    def start_service(self, name):
        ...
        threading.Thread(target=lambda: self._svc_op('start', name), daemon=True).start()
    ...
    def stop_service(self, name):
        ...
        threading.Thread(target=lambda: self._svc_op('stop', name), daemon=True).start()

    def restart_service(self, name):
        self.log(f"重启 {name}...", "info")
        threading.Thread(target=lambda: self._svc_op('restart', name), daemon=True).start()
    ...
    def _build_dashboard_page(self, page):
        scroll = tk.Canvas(page, bg=COLORS['bg_primary'], highlightthickness=0)
        scrollbar = ttk.Scrollbar(page, orient="vertical", command=scroll.yview)
        inner = tk.Frame(scroll, bg=COLORS['bg_primary'])
    ...
    def _make_scrollable_frame(self, parent):
        """Create a canvas-based scrollable container for long form pages"""
        canvas = tk.Canvas(parent, bg=COLORS['bg_primary'], highlightthickness=0)
        sb = ttk.Scrollbar(parent, orient="vertical", command=canvas.yview)
        inner = tk.Frame(canvas, bg=COLORS['bg_primary'])

        # Wait for inner frame to resize, then update canvas scrollregion
        inner.bind("<Configure>", lambda e: canvas.configure(scrollregion=canvas.bbox("all")))
        ...
        # Bind scroll wheel selectively when mouse enters canvas
    ...
    def _make_cfg_entry(self, parent, label, value, on_change, masked=False):
        r = tk.Frame(parent, bg=COLORS['bg_card'])
    ...
    def _build_cfg_channels(self, frame):
        ...
        # Cron
        cron_en = tk.BooleanVar(value=self.openclaw_data.get('cron', {}).get('enabled', False))
        tk.Checkbutton(misc_card, text="📅 定时任务 (Cron)", variable=cron_en,
                      bg=COLORS['bg_card'], fg=COLORS['text_primary'],
                      selectcolor=COLORS['bg_primary'], font=FONTS['body'],
                      activebackground=COLORS['bg_card'],
                      command=lambda: self._update_nested('cron', 'enabled', cron_en.get())
                      ).pack(anchor=tk.W)
