...
function parseSvcConfig(svc) {
  return {
    ...
    healthCheckGracePeriod: svc.healthCheckGracePeriod || 60000,
    healthCheckMaxFailures: svc.healthCheckMaxFailures || 5,
...
function loadServicesConfig() {
  ...
  try {
    ...
    for (const [name, svc] of Object.entries(config.services)) {
      ...
      services[name] = parseSvcConfig(svc);
    }
...
function getServiceLog(name, lines = 30) {
  ...
  // 反向读取：避免大文件全量加载导致内存峰值
  try {
    const stat = fs.statSync(logFile);
    const fileSize = stat.size;
    if (fileSize === 0) return '(无日志)';
    const chunkSize = Math.min(fileSize, 128 * 1024); // 最多读末尾 128KB
    ...
    // 第一行可能是截断的，去掉
    const cleanLines = chunkSize < fileSize ? allLines.slice(1) : allLines;
    return cleanLines.slice(-lines).join('\n');
  } catch (e) {
...
function startHealthCheckMonitor() {
  healthCheckTimer = setInterval(async () => {
    for (const [name, svc] of Object.entries(SERVICES)) {
      ...
      if (svc.healthCheck) {
        // 启动宽限期：给慢启动服务足够时间初始化
        const gracePeriod = svc.healthCheckGracePeriod || 60000;
        const timeSinceStart = state.lastStart ? (Date.now() - state.lastStart) : Infinity;
        if (timeSinceStart < gracePeriod) {
          continue; // 仍在宽限期内，跳过健康检查
        }
        ...
        if (health.status === 'healthy') {
          ...
          if (state.healthCheckFailures > 0) {
            log(`💚 ${svc.name} 恢复健康 (之前连续失败 ${state.healthCheckFailures} 次)`, 'INFO');
          }
          state.healthCheckFailures = 0;
          continue;
        }
        ...
        if (health.status === 'unhealthy') {
          state.healthCheckFailures = (state.healthCheckFailures || 0) + 1;
          const uptime = state.lastStart ? Math.round((Date.now() - state.lastStart) / 1000) : 0;
          const maxFailures = svc.healthCheckMaxFailures || 5;
          log(`⚠️ ${svc.name} 健康检查失败 (运行时间: ${uptime}s, 连续失败: ${state.healthCheckFailures}/${maxFailures})`, 'WARN');

          // 连续失败次数未达阈值，等待下次检查
          if (state.healthCheckFailures < 2) {
            log(`⏳ ${svc.name} 首次健康检查失败，等待下次检查确认...`, 'INFO');
            continue;
          }

          // 检查是否超过最大健康检查失败次数
          if (state.healthCheckFailures >= maxFailures) {
            log(`🛑 ${svc.name} 连续健康检查失败 ${state.healthCheckFailures} 次，超过上限 ${maxFailures}，停止自动恢复，等待冷却`, 'ERROR');
            state.stopping = true; // 防止 exit 回调触发 autoRestart
            state.status = 'stopped';
            state.pid = null;
            state.restarts = svc.maxRestarts; // 触发 maxRestarts 冷却逻辑
            state.maxRestartReachedAt = Date.now();
            state.healthCheckFailures = 0;
            try { process.kill(state.proc.pid, 'SIGKILL'); } catch (e) { }
            // 冷却完成后由健康检查恢复，此时重置 stopping
            setTimeout(() => { if (state.stopping) state.stopping = false; }, (svc.restartBackoffMax || 60000) + 5000);
            continue;
          }
...
function startService(name, options = {}) {
  ...
  // 如果服务有 pending 的 restartTimer，说明正在退避等待中，不重复启动
  if (processes[name]?.restartTimer && !options.force) {
    log(`${svc.name} 正在等待退避重启，跳过重复启动`, 'INFO');
    return true; // 返回 true 表示 "正在恢复中"，不阻塞依赖方
  }
  ...
  const state = processes[name] || { restarts: 0, healthCheckFailures: 0 };
  state.proc = proc;
  state.status = 'running';
  state.lastStart = Date.now();
  state.pid = proc.pid;
  state.healthCheckFailures = 0; // 重置健康检查失败计数
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      switch (action) {
        ...
        case 'health': {
          res.end(JSON.stringify({ status: 'ok', pm: 'sideria-pm v2.0' }));
          break;
        }
        case 'enable': {
          if (!target) {
          ...
          SERVICES[target] = parseSvcConfig(svc);
    ...
    lockServer.listen(PM_LOCK_PORT, '127.0.0.1', () => {
      ...
      const bootServices = () => {
        ...
        if (target) {
          ...
          // 按 startupOrder 排序服务（全量启动时默认跳过 live2d）
          const EXCLUDED_FROM_BULK_START = new Set(['live2d']);
